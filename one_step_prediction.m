...
for i=2:n_est
    ...
    for j=1:n_est-i+1
        index_mat = i+j-1; 
        x_hat_ik = x_hat_ik - theta_mat(:,(index_mat-1)*n_y+1:index_mat*n_y)*Y(:,idx-j) +...
                              theta_mat(:,n_est*n_y+(index_mat-1)*n_u+1:n_est*n_y+index_mat*n_u)*U(:,idx-j);
    end
    x_hat_k(:,i) = x_hat_ik;
end

[A_k,B_k,~,~] = pcac_matrices(theta,params);
