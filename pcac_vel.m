function u_pcac = pcac_vel(idx, Y, U, theta, params)
...
%% Gamma and T
A_tilde = [A_k,zeros(n_y*n_est,n_y+n_y*n_est+n_u);
        C_t*C_k*A_k,eye(n_y),zeros(n_y,n_y*n_est+n_u);
        eye(n_y*n_est),zeros(n_y*n_est,n_y),eye(n_y*n_est),zeros(n_y*n_est,n_u);
        zeros(n_u,2*n_y*n_est+n_y),eye(n_u)];

B_tilde = [B_k;C_t*C_k*B_k;zeros(n_y*n_est,n_u);eye(n_u)];
...
lb_x = [-inf+zeros(n_y*n_est,1);-inf+zeros(n_y,1);-inf+zeros(n_y*n_est,1);u_min];
ub_x = [inf+zeros(n_y*n_est,1);inf+zeros(n_y,1);inf+zeros(n_y*n_est,1);u_max];
lb_u = delta_u_min;
ub_u = delta_u_max;

x0 = [x_1-x_0;C_t*C_k*x_1-r_kl;x_1;u_k];

Q = blkdiag(zeros(n_y*n_est),Q_bar,zeros(n_y*n_est),zeros(n_u));
P = blkdiag(zeros(n_y*n_est),P_bar,zeros(n_y*n_est),zeros(n_u));

u_pcac = u_k + MPC(A_tilde,B_tilde,C_tilde,x0,A_ineq_y,B_ineq_y,A_ineq_u,B_ineq_u,lb_x,ub_x,lb_u,ub_u,Q,P,R,l);
